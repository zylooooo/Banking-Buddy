...
resource "aws_elastic_beanstalk_application" "transaction_service" {
  name        = "${var.name_prefix}-transaction-service"
  description = "Banking Buddy Transaction Service"
...
resource "aws_elastic_beanstalk_environment" "transaction_service" {
  name                = "${var.name_prefix}-transaction-env"
  ...
  setting {
    namespace = "aws:elasticbeanstalk:application:environment"
    name      = "DB_URL"
    value     = "jdbc:mysql://${split(":", var.rds_endpoint)[0]}:3306/crm_transactions?createDatabaseIfNotExist=true&useSSL=false&allowPublicKeyRetrieval=true&serverTimezone=UTC"
  }
  ...
  setting {
    namespace = "aws:elasticbeanstalk:application:environment"
    name      = "CRM_TRANSACTIONS_DB_SECRET_NAME"
    value     = var.crm_transactions_db_secret_name
  }
