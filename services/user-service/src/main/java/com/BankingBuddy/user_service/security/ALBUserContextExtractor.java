...
import com.BankingBuddy.user_service.exception.UnauthorizedException;
import com.BankingBuddy.user_service.config.AppProperties;
...
public class ALBUserContextExtractor {
    ...
    public UserContext extractUserContext(HttpServletRequest request) {
        ...
        // Try standard Authorization Bearer token (API Gateway)
    ...
    /**
     * Extract user context from standard JWT Bearer token (API Gateway)
     */
    private UserContext extractFromJWT(String jwtToken) {
        try {
            // Decode JWT (API Gateway already validated it)
    ...
    /**
     * Build UserContext from JWT claims (works for both ALB and API Gateway JWT)
     */
